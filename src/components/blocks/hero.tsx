...
const GalaxyCanvas = () => {
  ...
  useEffect(() => {
    ...
    const paintTile = (tw: number, th: number): HTMLCanvasElement => {
      ...
      purples.forEach(([fx,fy]) => {
        blob(fx*tw, fy*th, th*(0.20+Math.abs(Math.sin(fx*6))*0.12), [90,30,160],  0.38);
        blob(fx*tw, fy*th, th*(0.10+Math.abs(Math.cos(fx*8))*0.06), [130,50,200], 0.30);
      });
      ...
      violets.forEach(([fx,fy]) => {
        blob(fx*tw, fy*th, th*(0.12+Math.abs(Math.sin(fx*10))*0.07), [70,20,130],  0.35);
        blob(fx*tw, fy*th, th*0.05,                                   [180,100,255],0.20);
      });

      // ── Pink-purple emission (where blue & purple meet) ───────────────────
      const pinks = [
        [0.30,0.55],[0.60,0.35],[1.00,0.60],[1.35,0.38],[1.75,0.55],
        [2.10,0.35],[2.55,0.58],[2.85,0.40],
      ];
      pinks.forEach(([fx,fy]) => {
        blob(fx*tw, fy*th, th*0.07, [160,60,180], 0.22);
        blob(fx*tw, fy*th, th*0.03, [210,120,255],0.20);
      });

      // ── Teal-green accents ────────────────────────────────────────────────
      const teals = [[0.35,0.25],[0.70,0.65],[1.15,0.25],[1.55,0.65],[2.0,0.30],[2.50,0.65]];
      teals.forEach(([fx,fy]) => {
        blob(fx*tw, fy*th, th*0.09, [20,140,140], 0.18);
        blob(fx*tw, fy*th, th*0.04, [80,200,200], 0.15);
      });

      // ── White core sparkle regions ────────────────────────────────────────
      const whites = [[0.22,0.45],[0.62,0.52],[1.08,0.42],[1.52,0.50],[2.05,0.48],[2.55,0.44]];
      whites.forEach(([fx,fy]) => {
        blob(fx*tw, fy*th, th*0.05, [240,235,255], 0.22);
      });
