...
const GalaxyCanvas = () => {
  ...
  useEffect(() => {
    ...
    const makeBlobs = (): Blob[] => {
      ...
      // ── TOP edge blobs ────────────────────────────────────────────────────
      [[0.05,0.04],[0.18,0.08],[0.32,0.02],[0.50,0.07],[0.65,0.03],
       [0.80,0.09],[0.95,0.05],[1.10,0.02],[1.28,0.08],[1.45,0.04],
       [1.62,0.07],[1.78,0.02],[1.95,0.09],[2.12,0.05],[2.30,0.03],
       [2.48,0.08],[2.65,0.04],[2.82,0.07],[2.96,0.02],
      ].forEach(([fx,fy]) => {
        push(fx,fy, 0.22+Math.abs(Math.sin(fx*8))*0.14, [18,60,160],  0.44, true);
        push(fx,fy, 0.12+Math.abs(Math.cos(fx*6))*0.08, [90,30,160],  0.32);
        push(fx,fy, 0.08, [50,110,220], 0.28);
      });

      // ── BOTTOM edge blobs ─────────────────────────────────────────────────
      [[0.08,0.96],[0.22,0.92],[0.38,0.97],[0.55,0.91],[0.70,0.95],
       [0.85,0.98],[1.02,0.93],[1.18,0.97],[1.35,0.91],[1.52,0.96],
       [1.68,0.93],[1.85,0.98],[2.02,0.92],[2.20,0.96],[2.38,0.98],
       [2.55,0.93],[2.72,0.97],[2.88,0.91],[2.98,0.95],
      ].forEach(([fx,fy]) => {
        push(fx,fy, 0.22+Math.abs(Math.sin(fx*7))*0.14, [30,80,200],  0.44, true);
        push(fx,fy, 0.12+Math.abs(Math.cos(fx*9))*0.08, [70,20,130],  0.32);
